...
public record TaskCreateDTO([Required, StringLength(100)]string Title, int? AssignedToId, string? Description, ICollection<string> Tags);

public record TaskUpdateDTO(int Id, [Required, StringLength(100)]string Title, int? AssignedToId, string? Description, ICollection<string> Tags, State State);
